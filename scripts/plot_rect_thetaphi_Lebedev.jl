...
# 球面采样点与权重
nodes, weights = getlbSortedData(τ)
...
# 球面采样点与权重
nodesf, _ = getlbSortedData(τf)
# 转换到球坐标
rθϕnodesf = reduce(hcat, map(cart2sphere, eachcol(nodesf)))

fig = viz_data_in_thetaphi_plane(rθϕnodes[[3,2], :], rθϕnodesf[[3,2], :], title = L"(\tau = %$(τ)) to (\tau = %$(τf))", filename = "Levedevθϕnodesk2f", size_inches = (3.2, 2.4), fontsize = 10)
...
rθϕnodes = reduce(hcat, map(cart2sphere, eachcol(nodes)))
fig = viz_data_in_thetaphi_plane(rθϕnodes[[3,2], :], title = L"\tau = %$(τ)", filename = "LebedevθϕnodesLebedev32", size_inches = (3.2, 2.4), fontsize = 10)
